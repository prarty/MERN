    ...
    const handleImageSubmit = (e) => {
        ...
        if(files.length > 0 && (files.length+formData.imageUrls.length) < 7) {
            ...
            Promise.all(promises).then((urls) => {
                ...
                setImageUploadError('Image upload error max 2mb')
                setUploading(false)
            })
        } else {
        ...
        <main className='p-3 max-w-4xl mx-auto'>
            ...
            <form className='flex flex-col sm:flex-row gap-4'>
                <div className='flex flex-col gap-4 flex-1'>
                    <input type='text' placeholder='Name' id='name' className='border p-3 rounded-lg' maxLength='62'
                    ...
                    <div className='flex flex-wrap gap-6'>
                        <div className='flex items-center gap-2'>
                            <input className='p-3 border border-gray-300 rounded-lg' type='number' id='bedrooms' min='1'
                                   max='10' required/>
                            <span>Beds</span>
                        </div>
                        <div className='flex items-center gap-2'>
                ...
                <div className='flex flex-col flex-1 gap-4'>
                    ...
                    <div className='flex gap-4'>
                        <input onChange={(e) => setFiles(e.target.files)}
                               className='p-3 border border-gray-300 rounded-lg' type='file' id='images'
                               accept='image/*' multiple/>
                        <button type='button' onClick={handleImageSubmit}
                                className='p-3 text-green-700 border border-green-700 rounded uppercase
                            hover:shadow-lg disabled:opacity-80'>{uploading ? 'Uploading...' : 'Upload'}
                        </button>
                    </div>
                    ...
                    <button
                        className='p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80'>Create
                        Listing
                    </button>
                </div>
            </form>
